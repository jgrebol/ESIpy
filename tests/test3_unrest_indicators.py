...
expected = {'mulliken': dict(exp_pop_atm1_a=3.1944, exp_pop_atm1_b=2.7968, exp_di_all_a=0.9840, exp_di_all_b=0.8946,
                             ...
                             exp_iring=0.007366, exp_mci=-0.023448, exp_av=1.165, exp_pdi=0.062509),
    'lowdin': dict(exp_pop_atm1_a=3.1460, exp_pop_atm1_b=2.7565, exp_di_all_a=0.9936, exp_di_all_b=0.9000,
                     ...
                     exp_iring=0.007371, exp_mci=-0.023151, exp_av=1.126, exp_pdi=0.062565),
    ...
    'nao': dict(exp_pop_atm1_a=3.1291, exp_pop_atm1_b=2.7418, exp_di_all_a=0.9915, exp_di_all_b=0.8968,
                                ...
                                exp_iring=0.007423, exp_mci=-0.022291, exp_av=1.094, exp_pdi=0.062161),
    'iao': dict(exp_pop_atm1_a=3.1922, exp_pop_atm1_b=2.8019, exp_di_all_a=0.9852, exp_di_all_b=0.8925,
                        ...
                        exp_iring=0.007212, exp_mci=-0.023300, exp_av=1.102, exp_pdi=0.059074)}
...
class ESItest(unittest.TestCase):
    ...
    def run_pop_tests(self, partition, exp):
        ...
        self.assertAlmostEqual(trace(aom[0][0]), exp_pop_atm1_a, places=4)
        self.assertAlmostEqual(trace(aom[1][0]), exp_pop_atm1_b, places=4)
        ...
        self.assertAlmostEqual(di_1_all_a, exp_di_all_a, places=4)
        self.assertAlmostEqual(di_1_all_b, exp_di_all_b, places=4)
        ...
        self.assertAlmostEqual(sum(dis_a), exp_dis_sum_a, places=4)
        self.assertAlmostEqual(sum(dis_b), exp_dis_sum_b, places=4)
        self.assertAlmostEqual(sum(lis_a) + sum(lis_b) + sum(dis_a) + sum(dis_b), 42, places=4)
        # Testing DI between C1 and C2
        self.assertAlmostEqual(2 * trace(dot(aom[0][0], aom[0][1])), exp_di_12_a, places=4)
        self.assertAlmostEqual(2 * trace(dot(aom[1][0], aom[1][1])), exp_di_12_b, places=4)
    ...
    def run_indicator_tests(self, partition, exp):
        ...
        self.assertAlmostEqual((compute_iring(ring, aom[0]) + compute_iring(ring, aom[1])), exp_iring, places=6)
        self.assertAlmostEqual((sequential_mci(ring, aom[0], partition=partition) + sequential_mci(ring, aom[1], partition=partition)), exp_mci, places=5)
        self.assertAlmostEqual((multiprocessing_mci(ring, aom[0], ncores=1, partition=partition) + multiprocessing_mci(ring, aom[1], ncores=1, partition=partition)), exp_mci, places=5)
        self.assertAlmostEqual((compute_av1245(ring, aom[0])[0] + compute_av1245(ring, aom[1])[0]), exp_av, places=2)
        self.assertAlmostEqual((compute_pdi(ring, aom[0])[0] + compute_pdi(ring, aom[1])[0]), exp_pdi, places=6)
