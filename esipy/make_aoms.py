...
def make_aoms(mol, mf, partition, myhf=None, save=None):
    ...
    # 2. RESTRICTED
        ...
        if is_natorb:
            mask = occ > 1e-10
            if not hasattr(mf, 'mo_occ'):
                 _, coeff_src = get_natorbs(mf, S)
            coeff = coeff_src[:, mask]
            occ_mask = occ[mask]
            
            # Special IAO handling for natorbs if it's an IAO-like partition
            if "iao" in partition_label or "piao" in partition_label:
                try:
                    from esipy.iao import reference_mol
                    pmol = reference_mol(mol, source_basis="minao")
                    n_ref = pmol.nao
                    # Note: we might need a specialized get_iao_aoms here or in iao.py
                    aom = get_iao_aoms(partition_label, coeff_src[:, :n_ref], mf)
                except ImportError:
