    ...
    while i < len(lines):
        ...
        if line.startswith('ELEMENT'):
            ...
            if len(parts) >= 3 and parts[2].upper() == symbol.upper():
                ...
                while i < len(lines) and 'NSHELL' not in lines[i]:
                    i += 1
                ...
                if i < len(lines):
                    ...
                    for _ in range(num_shells):
                        while i < len(lines) and 'SHELL' not in lines[i]:
                            i += 1
...
def get_reference_basis_dict(mol, source_basis='minao', pol_basis=None, x=1.0, heavy_only=False, full_basis=False):
    def get_minimal_part(sym, source_basis_name):
        ...
        for shell in basis_source:
            ...
            current_l_count = l_counts.get(l, 0)
            needed_l_count = target_l.get(l, 0)
            
            if current_l_count < needed_l_count:
                n_to_take = min(n_contractions, needed_l_count - current_l_count)
                
                if n_to_take == n_contractions:
                    new_basis_atom.append(shell)
                else:
                    new_shell = [l]
                    for prim in shell[1:]:
                        new_shell.append(prim[:n_to_take+1])
        ...
        for shell in basis_pol:
            ...
            if l in target_pol_l and l not in target_l:
                ...
                if current_l_count < needed_l_count:
                    ...
                    for prim in shell[1:]:
                        scaled_exp = prim[0] * x
                        new_shell.append([scaled_exp] + list(prim[1:n_to_take+1]))
...
def reference_mol(mol, polarized=False, pol_basis=None, source_basis='minao', x=1.0, heavy_only=False, full_basis=False):
    ...
    if not isinstance(mol, gto.Mole):
        pyscf_mol = getattr(mol, 'pyscf_mol', None)
        if pyscf_mol is not None:
            mol = pyscf_mol
        else:
            mol_attr = getattr(mol, 'mol', None)
    ...
    ref_basis = get_reference_basis_dict(
        mol, source_basis=source_basis, pol_basis=pol_basis, 
        x=x, heavy_only=heavy_only, full_basis=full_basis
    )
...
def iao(mol, coeffs, source_basis='minao', heavy_only=False, full_basis=False):
    ...
    pmol = reference_mol(
        mol, polarized=False, source_basis=source_basis, 
        heavy_only=heavy_only, full_basis=full_basis
    )
...
def fpiao(mol, coeffs, x=1.0, source_basis='minao', pol_basis='ano', heavy_only=True, full_basis=False):
    ...
    pmol = reference_mol(
        mol, polarized=True, pol_basis=pol_basis, 
        source_basis=source_basis, x=x, 
        heavy_only=heavy_only, full_basis=full_basis
    )
...
def autosad(mol, coeffs, polarized=False, heavy_only=False, full_basis=False, x=1.0):
    ...
    w, A, pmol = get_effaos(
        mol, coeffs, free_atom=True, polarized=polarized, 
        heavy_only=heavy_only, full_basis=full_basis, x=x
    )
...
def effao(mol, coeffs, mode='net', polarized=False, heavy_only=False, full_basis=False, x=1.0):
    ...
    w, A, pmol = get_effaos(
        mol, coeffs, free_atom=False, mode=mode, 
        polarized=polarized, heavy_only=heavy_only, 
        full_basis=full_basis, x=x
    )
...
def wiao(mol, coeffs, heavy_only=False, full_basis=False):
    ...
    for i in range(natm):
        for j in range(natm):
            if mol.atom_pure_symbol(i) == 'H' or mol.atom_pure_symbol(j) == 'H':
                ...
                B_atom[i, j] = np.sum(B_whole[ao_loc[i]:ao_loc[i+1], ao_loc[j]:ao_loc[j+1]])
    ...
    _, A_tilde, pmol = get_effaos(
        mol, coeffs, free_atom=False, mode='meta-lowdin', 
        polarized=False, heavy_only=heavy_only, full_basis=full_basis
    )
