...
class MeanField2:
    ...
    def __init__(self, path, mole2: Mole2):
        ...
        nalpha = int(getattr(self.mole2.fchk, 'nalpha', 0))
        nbeta = int(getattr(self.mole2.fchk, 'nbeta', 0))
        if nalpha == nbeta:
            # Restricted
        ...
        if wf == 'rest':
            ...
            nummo = int(getattr(self.mole2.fchk, 'nummo', len(mo_flat) // int(self.mole2.nao)))
            numao = int(self.mole2.nao)
            mo_arr = np.array(mo_flat, dtype=float).reshape(nummo, numao).T
        else:
        ...
        # Now permute rows so FCHK (Gaussian) AO ordering -> PySCF AO ordering
        # Use the Mole2 object which has self._bas/_env representing the FCHK
        # Gaussian ordering. Permutation is always applied.
        mol_for_perm = self.mole2
        if isinstance(mo_arr, list):
            self.mo_coeff = [permute_aos_rows(mo_arr[0], mol_for_perm), permute_aos_rows(mo_arr[1], mol_for_perm)]
        else:
            self.mo_coeff = permute_aos_rows(mo_arr, mol_for_perm)

        # Attach MO coefficients to pyscf SCF object
        try:
            self._scf.mo_coeff = self.mo_coeff
        except Exception:
            # pyscf expects a particular shape; try to set attribute directly
            setattr(self._scf, 'mo_coeff', self.mo_coeff)

        # Expose convenience attributes
        self.mo_occ = getattr(self.mole2.fchk, 'mo_occ', getattr(self.mole2.fchk, 'mo_occ', None))
        self.nbasis = self.mole2.nao
        self.nao = int(self.nbasis)
...
class FchkMolecule:
    def __init__(self, path):
        ...
        # Do NOT build _bas/_env/_atm here: this object is only a reader of
        # FCHK arrays. The Mole2 wrapper will call make_bas_env() when it
        # needs the Gaussian-order _bas/_env/_atm.
