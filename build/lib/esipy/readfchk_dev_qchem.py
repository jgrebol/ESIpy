...
class MeanField2:
    ...
    def __init__(self, path, mole2: Mole2):
        ...
        theory_tokens = read_level_theory(self.path)
        is_unrestricted = False
        for tok in theory_tokens:
        ...
        if wf == 'rest':
            ...
            self.mo_coeff = (mo_arr_a, mo_arr_b)

            mo_occ_a = np.zeros(self.nummo)
            mo_occ_b = np.zeros(self.nummo)
            mo_occ_a[:self.nalpha] = 1.0
            mo_occ_b[:self.nbeta] = 1.0
    ...
    def make_rdm1(self, ao_repr=True):
        ...
        if self.mo_coeff is None:
            return None
        if isinstance(self.mo_coeff, tuple):
            # UHF
            ca, cb = self.mo_coeff
...
class FchkMolecule:
    def __init__(self, path):
        ...
        if is_qchem:
            ...
            if ovlp_flat:
                # Q-Chem stores it as a flat list, likely lower triangular or full
                # For H2O sto-3g, nbasis=7, N=28. 28 = 7*(7+1)/2. So it's symmetric lower triangular.
