...
def make_aoms(mol, mf, partition, myhf=None, save=None, is_fchk=False):
    ...
    def get_iao_aoms(p_type, c, current_mf, w_override=None, current_myhf=None, c_full=None):
        ...
        print(f"DEBUG: c_full is None? {c_full is None}")
        print(f"DEBUG: proj_c shape = {proj_c.shape}, U shape = {U.shape}, eta shape = {eta[0].shape}")
    ...
    if is_unrest:
        ...
        if partition_label in ("lowdin", "meta-lowdin", "nao", "mulliken"):
            ...
            if is_natorb:
                ...
                aom_alpha = get_iao_aoms(partition_label, ca[:, mask_a], mf, c_full=ca)
                aom_beta = get_iao_aoms(partition_label, cb[:, mask_b], mf, c_full=cb)
                aom = [aom_alpha, aom_beta]
    ...
    # 2. RESTRICTED
        ...
        if partition_label in ("lowdin", "meta-lowdin", "nao", "mulliken"):
            ...
            if is_natorb:
                ...
                aom = get_iao_aoms(partition_label, coeff, mf, c_full=c_full)
