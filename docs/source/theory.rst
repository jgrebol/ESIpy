...
separating the atomic basis functions belonging to a certain atom). The ESI-3D code [1]_ developed by Dr. Eduard Matito
mainly used Bader's QTAIM [2]_ (real-space scheme) as the AIM for the calculations. However, in this program we propose
...
where we can introduce the elements of the P-matrix, :math:`P_{\nu\mu} = 2 \sum_i ^{nocc} c_{\nu i} c_{i\mu}^+`, and
the overlap matrix in the Atomic Orbitals (AO, :math:`\chi` )
...
Moreover, the Delocalization Index (DI, $\delta$), also referred to as Bond Order (BO) [9]_, measures the average number
...
in a 6-membered ring, so the **para-delocalization index (PDI)** [10]_:
...
Giambiagi and coworkers proposed to express an index in terms of the generalized bond order along the ring, the **I\ :sub:`ring`** [11]_. That is, to account for the delocalization along the ring, following the specified connectivity:
...
generated by permuting the position of all atoms in the ring, denoted as :math:`\mathscr{P}(\mathscr{A})`:
...
The formula depends on a series of tabulated :math:`R_{opt}`, as well as the normalization factor :math:`\alpha` for each bond to
...
HOMER aromaticity index is a reparametrization of the HOMA for the T1 state [16]_. Different parameters can be introduced
