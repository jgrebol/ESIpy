...
ESIpy can read a FCHK file to get the wavefunction information generated by Gaussian and Q-Chem.
...
..  note::
    ...
    Refer to the :doc:`installation` page for instructions on how to install ESIpy from the GitHub repository.
...
- $FINDRING: If this keyword is present, ESIpy will search for all the rings in the molecule using its ring-finding algorithm.
- $NORING: No ring specified.

The ring-finding algorithm looks for all the closed circuits within a minimum and maximum length, defined by the $MAXLEN and $MINLEN keywords. Also, $EXCLUDE can be used to not consider a certain atom (by its index or atomic symbol) during the ring finding algorithm.
...
More than one partition can be specified at a time, in different lines. Appart from $PARTITION, one can use $ALLPARTS as an equivalent way to set $PARTITION to ALL.
...
- $FLUREF: A list of reference indices to compute the FLU index, following the ring connectivity. For example "CC 1.5000" on the line below.
...
- $FRAGMENTS: Define fragments for the computation of the delocalization indices. The structure is Number of fragments, Number of atoms in fragment 1, Atom indices in fragment 1, Number of atoms in fragment 2, Atom indices in fragment 2, etc.
- $ECP: Needed if the calculation uses ECP. The line below must have the name of the ECP.
