...
def aromaticity_from_aoms(Smo, rings, calc=None, wf=None, mci=False, av1245=False, num_threads=None):
   ...
   # UNRESTRICTED
   if wf == "unrest": print(' Wavefunction type:        uhf') 

   # RESTRICTED
   if wf == "rest": print(' Wavefunction type:        hf')

   if wf != "unrest" and wf != "rest": raise NameError("Please insert a valid type of wave function: 'rest' or 'unrest'") 

   if calc is not None: print(' Atomic partition:         {}'.format(calc.upper()))
   if calc is None: print(' Atomic partition:         Not specified')
   ...
   # UNRESTRICTED
   if wf == 'unrest':
      arom_unrest_from_aoms(Smo, rings, mci = mci, av1245 = av1245, num_threads = num_threads)
...
def deloc_unrest(mol, Smo):
  ...
  for i in range(mol.natm):
     print(' | {} {:>2d}   {:10.6f}  {:10.6f}  {:10.6f}   *******   {:8.4f}   {:8.4f} '.format(mol.atom_symbol(i), i+1, Nij_alpha[i] + Nij_beta[i], Nij_alpha[i], Nij_beta[i], lis_alpha[i], lis_beta[i]))
  print(' ----------------------------------------------------------------------------- ')
  print(' | TOT:   {:10.6f}  {:10.6f}  {:10.6f}   *******   {:8.4f}   {:8.4f}'.format(sum(Nij_alpha)+sum(Nij_beta), sum(Nij_alpha), sum(Nij_beta), sum(lis_alpha), sum(lis_beta)))
  print(' ----------------------------------------------------------------------------- ')
  print(' ------------------------------------------- ')
  ...
  for i in range(mol.natm):    
     for j in range(mol.natm):
        if i != j:
           print(' | {} {}-{} {}  {:>9.4f} {:>9.4f} {:>9.4f}'.format(mol.atom_symbol(i), str(i + 1).rjust(2), mol.atom_symbol(j), str(j + 1).rjust(2), 2*(np.trace(np.dot(Smo[0][i], Smo[0][j])) + np.trace(np.dot(Smo[1][i], Smo[1][j]))), 2*(np.trace(np.dot(Smo[0][i], Smo[0][j]))),  2*(np.trace(np.dot(Smo[1][i], Smo[1][j])))))
  print(' ------------------------------------------- ')
...
def deloc_rest(mol, Smo):
   ...
   for i in range(mol.natm):
      if i != j:
         print(' | {} {:>2d}    {:10.6f}     *******   {:8.4f}   {:8.4f} '.format(mol.atom_symbol(i), i+1, Nij[i], lis[i], Nij[i]-lis[i]))
   print(' ------------------------------------------------------- ')
   print(' | TOT:    {:10.6f}     *******   {:8.4f}   {:8.4f}'.format(sum(Nij), sum(Nij)-sum(dis), sum(dis)))
   print(' ------------------------------------------------------- ')
   ...
   for i in range(mol.natm):
      for j in range(mol.natm):
         if i != j:
            print(' | {} {}-{} {}   {:8.4f}'.format(mol.atom_symbol(i), str(i + 1).rjust(2), mol.atom_symbol(j), str(j + 1).rjust(2), 4*np.trace(np.dot(Smo[i], Smo[j]))))
   print(' ------------------------ ')
         ...
         if len(ring) < 6:
            ...
            for j in range(len(ring)):
               print(' |  A {} - A {} - A {} - A {}  |  {:>9.4f}'.format(
                  str(av_order[j][0]).rjust(2), str(av_order[j][1]).rjust(2), str(av_order[j][2]).rjust(2), str(av_order[j][3]).rjust(2),np.array(avs_alpha[2][(ring[j] - 1) % len(ring)])
               ))
            print(' |   AV1245_alpha {} =             {:>9.4f}'.format(ring_index + 1, avs_alpha[0]))
            print(' |    AVmin_alpha {} =             {:>9.4f}'.format(ring_index + 1, avs_alpha[1]))
            ...
            for j in range(len(ring)):
               print(' |  A {} - A {} - A {} - A {}  |  {:>9.4f}'.format(
                  str(av_order[j][0]).rjust(2), str(av_order[j][1]).rjust(2), str(av_order[j][2]).rjust(2), str(av_order[j][3]).rjust(2),np.array(avs_beta[2][(ring[j] - 1) % len(ring)])
               ))
            print(' |   AV1245_beta  {} =             {:>9.4f}'.format(ring_index + 1, avs_beta[0]))
            print(' |    AVmin_beta  {} =             {:>9.4f}'.format(ring_index + 1, avs_beta[1]))
            print(' |')
         ...
         if len(ring) < 6:
            ...
            for j in range(len(ring)):
               print(' |  A {} - A {} - A {} - A {}  |  {:>6.4f}'.format(
                  str(av_order[j][0]).rjust(2), str(av_order[j][1]).rjust(2), str(av_order[j][2]).rjust(2), str(av_order[j][3]).rjust(2), avs[2][(ring[j] - 1) % len(ring)]
               ))
            print(' | AV1245 {} =             {:.4f}'.format(ring_index + 1, avs[0]))
            print(' |  AVmin {} =             {:.4f}'.format(ring_index + 1, avs[1]))
            print(' ---------------------------------------------------------------------- ')
