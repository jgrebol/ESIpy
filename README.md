...
As well as the previous indices, a larger MCI value denotes a more aromatic character. Due to the exponential growth of the calculation, we do not suggest computing the MCI for rings larger than $n$=12 for single-core processes and $n$=14 for multi-core processes. See [ESIpy/MCI_TIMINGS](ESIpy/MCI_TIMINGS) for details and timings of the algorithms.
